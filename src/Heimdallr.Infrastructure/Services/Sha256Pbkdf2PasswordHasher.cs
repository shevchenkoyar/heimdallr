...
{
    public ValueTask<string> HashAsync(string login, string password, CancellationToken cancellationToken)
    {
        byte[] passwordAsBytes = Encoding.UTF8.GetBytes(password);

        byte[] salt = GenerateSalt(login);
    ...
    private byte[] GenerateSalt(string login)
    {
        int prevHash = login.GetHashCode();
        return BitConverter.GetBytes(prevHash);
    }
}
